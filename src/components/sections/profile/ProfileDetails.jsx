    ...
    return (
      <div className="flex flex-col gap-6 text-left">
        <Card variant="default" className="p-6 bg-[#1A1A1A]">
          <h3 className="font-serif text-lg font-bold text-[#F5F0EB] border-b border-zinc-800 pb-3 mb-4 flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs mb-6">
            <div className="flex flex-col gap-1">
              <span className="text-zinc-500 uppercase tracking-wider text-[10px] font-bold">{isChinese ? '吸烟习惯' : 'Smoking'}</span>
              <span className="text-[#F5F0EB]">{isChinese ? '从不吸烟' : 'Never'}</span>
            </div>
            <div className="flex flex-col gap-1">
              <span className="text-zinc-500 uppercase tracking-wider text-[10px] font-bold">{isChinese ? '饮酒习惯' : 'Drinking'}</span>
              <span className="text-[#F5F0EB]">{isChinese ? '社交性饮酒' : 'Socially'}</span>
            </div>
            <div className="flex flex-col gap-1">
              <span className="text-zinc-500 uppercase tracking-wider text-[10px] font-bold">{isChinese ? '生活作息' : 'Lifestyle Mode'}</span>
              <span className="text-[#F5F0EB]">{member.lifestyle || (isChinese ? '规律健康' : 'Balanced')}</span>
            </div>
          </div>

          <div className="flex flex-col gap-1.5 text-xs mb-6">
            <span className="text-zinc-500 uppercase tracking-wider text-[10px] font-bold">{isChinese ? '是否接受跨国/跨地区关系' : 'Open to International Relationship'}</span>
    ...
    <div className="flex flex-col gap-6 text-left">
      <Card variant="default" className="p-6 bg-[#1A1A1A]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs mb-6">
          <div className="flex flex-col gap-1">
            <span className="text-zinc-500 uppercase tracking-wider text-[10px] font-bold">{isChinese ? '期望关系类型' : 'Looking For'}</span>
            <span className="text-[#F5F0EB] font-semibold">{isChinese ? '严肃的婚姻/长期关系' : 'Serious Long-term Relationship'}</span>
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-zinc-500 uppercase tracking-wider text-[10px] font-bold">{isChinese ? '期望伴侣年龄范围' : 'Partner Age Range'}</span>
            <span className="text-[#F5F0EB]">{member.matchmakerReport?.idealMatchProfile?.ageRange || '30–42'}</span>
          </div>
        </div>
      </Card>
      ...
      <Card variant="default" className="p-6 bg-[#1A1A1A] border-[#C0392B]/15">
        <h3 className="font-serif text-lg font-bold text-[#F5F0EB] border-b border-zinc-800 pb-3 mb-4 flex items-center gap-2">
          <ShieldAlert className="w-4 h-4 text-[#C0392B]" />
          <span>{isChinese ? '伴侣底线 (Non-negotiables)' : 'Non-Negotiables'}</span>
        </h3>
        <ul className="list-disc pl-5 text-xs text-[#9A8F8A] leading-relaxed flex flex-col gap-2">
