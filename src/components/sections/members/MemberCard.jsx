...
export const MemberCard = ({ member, isLoggedIn, isChinese }) => {
  ...
  return (
    ...
    >
      <Card
        variant="glass"
        className="w-full p-6 flex flex-col justify-between items-center text-center relative border-[#0F8A96]/10 hover:border-[#0F8A96]/40 duration-300"
      >
        {/* Verification Shield overlay */}
        <div className="absolute top-4 left-4">
          <div className="flex items-center gap-1 text-[9px] font-bold uppercase tracking-widest text-zinc-450 bg-white/80 px-2 py-0.5 border border-zinc-200/80 shadow-sm rounded-full">
        ...
        <div className="flex-1 w-full flex flex-col items-center">
          <div className="flex items-center justify-center gap-2 mb-2 text-zinc-800 text-lg font-bold font-serif w-full">
          ...
          <div className="text-xs text-zinc-550 leading-relaxed mb-6 font-light">
            <span>{excerpt}</span>
            {remaining && (
        ...
        {/* Stats */}
        <div className="w-full grid grid-cols-2 gap-2 text-[10px] uppercase tracking-widest text-zinc-450 font-semibold border-t border-zinc-150 pt-4 mb-6">
          <div>
            <span className="block text-zinc-600 mb-0.5">{isChinese ? '身高' : 'Height'}</span>
            <span className="text-zinc-800 text-xs font-normal">{member.height}</span>
          </div>
          <div>
            <span className="block text-zinc-600 mb-0.5">{isChinese ? '最高学历' : 'Education'}</span>
            <span className="text-zinc-800 text-xs font-normal truncate block">{member.education}</span>
          </div>
        </div>
